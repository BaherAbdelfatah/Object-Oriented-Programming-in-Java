...
public class EarthquakeCityMap extends PApplet {
	...
	private void addKey() {	
		...
		fill(color(200, 0, 0));
		float x=60,y=100+10/3;
		float l=10;
		float h=(float) (l*Math.sqrt(3)*0.5);
		triangle(x-l/2, y+h/3, x+l/2, y+h/3, x, y-h);
		fill(0, 0, 0);
		...
		fill(255, 255, 0);
		ellipse(60, 200, 15, 15);
		fill(0, 0, 255);
		ellipse(60, 220, 15, 15);
		fill(255, 0, 0);
		...
		for (Marker cm : countryMarkers) {
			...
			for (Marker m : quakeMarkers)
			{
				...
				if(em.isOnLand==true)
				{
					...
					if((String)cm.getProperty("name")==country)
					{
						countryQuake_counter++;
					}
				}
				
			}
			if(countryQuake_counter >= 1)
			System.out.println((String)cm.getProperty("name")+": "+countryQuake_counter);
		}
